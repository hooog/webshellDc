...
class LanguageIC:
    ...
    def printer(self, count):
        ...
        for x in range(count):
            print
            ' {0:>7.4f}        {1}'.format(self.results[x]["value"], self.results[x]["filename"])
        return
...
class LongestWord:
    ...
    def printer(self, count):
        ...
        for x in range(count):
            print
            ' {0:>7}        {1}'.format(self.results[x]["value"], self.results[x]["filename"])
        return
...
class SignatureNasty:
    ...
    def calculate(self, data, filename):
        ...
        valid_regex = re.compile(
            '(eval\(|file_put_contents|base64_decode|python_eval|exec\(|passthru|popen|proc_open|pcntl|assert\(|system\(|shell)',
            re.I)
            ...
            for file in files:
                ...
                if (valid_regex.search(file) and os.path.getsize(filename) > SMALLEST):
                    try:
                        ...
                        data = False
                        print
                        "Could not read file :: %s/%s" % (root, file)
                    yield data, filename
...
if __name__ == "__main__":
    ...
    for x in range(count):
        print
        ' {0:>7}        {1}'.format(rank_sorted[x][1], rank_sorted[x][0])
